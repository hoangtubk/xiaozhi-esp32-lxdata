    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging))
    {
        ...
        if (low_battery_popup_ != nullptr)
        {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging)
            {
                ...
                { // Show if low battery popup is hidden
            ...
            else
            {
                ...
                if (!lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN))
                { // Hide if low battery popup is shown
...
bool LvglDisplay::SnapshotToJpeg(std::string &jpeg_data, int quality)
{
    ...
    bool ret = image_to_jpeg_cb((uint8_t *)draw_buffer->data, draw_buffer->data_size, draw_buffer->header.w, draw_buffer->header.h, PIX_FMT_RGB565, quality,
