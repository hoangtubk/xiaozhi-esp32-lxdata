...
static uint8_t *convert_input_to_encoder_buf(const uint8_t *src, uint16_t width, uint16_t height, pixel_format_t format,
                                             jpeg_pixel_format_t *out_fmt, int *out_size)
{
    // GRAY 直接作为 JPEG_PIXEL_FORMAT_GRAY 输入
    if (format == PIX_FMT_GREY)
    {
    ...
    // YUYV (Y Cb Y Cr) 可直接作为 JPEG_PIXEL_FORMAT_YCbYCr 输入
    if (format == PIX_FMT_YUYV)
    {
    ...
    // UYVY (Cb Y Cr Y) -> 重排为 YUYV 再作为 YCbYCr 输入
    if (format == PIX_FMT_UYVY) [[unlikely]]
    {
        int sz = (int)width * (int)height * 2;
        const uint8_t *s = src;
        uint8_t *buf = (uint8_t *)jpeg_calloc_align(sz, 16);
        ...
        {
            // src: Cb, Y0, Cr, Y1 -> dst: Y0, Cb, Y1, Cr (YUYV)
            d[0] = s[1];
            d[1] = s[0];
            d[2] = s[3];
            d[3] = s[2];
    ...
    // YUV422P (YUV422 Planar) -> 重排为 YUYV (YCbYCr)
    if (format == PIX_FMT_YUV422P) [[unlikely]]
    {
        int sz = (int)width * (int)height * 2;
    ...
    else if (format == PIX_FMT_RGB24 || format == PIX_FMT_RGB565 || format == PIX_FMT_RGB565X || format == PIX_FMT_RGB888)
    {
        ...
        case PIX_FMT_RGB565:
            ...
            break;
        [[unlikely]] case PIX_FMT_RGB565X: // big-endian variant
            in_pixel_fmt = ESP_IMGFX_PIXEL_FMT_RGB565_BE;
            src_len = static_cast<uint32_t>(width * height * 2);
            break;
        [[unlikely]] default:
            ESP_LOGE(TAG, "[Unreachable Case] unsupported format: 0x%08x", (unsigned)format);
            std::unreachable();
        }
        int sz = (int)width * (int)height * 2;
        uint8_t *buf = (uint8_t *)jpeg_calloc_align(sz, 16);
    ...
    ESP_LOGE(TAG, "unsupported format: 0x%08x", (unsigned)format);
    if (out_size)
        *out_size = 0;
    return nullptr;
}
...
static uint8_t *convert_input_to_hw_encoder_buf(const uint8_t *src, uint16_t width, uint16_t height, pixel_format_t format,
                                                jpeg_enc_input_format_t *out_fmt, int *out_size)
{
    ...
    if (format == PIX_FMT_RGB24 || format == PIX_FMT_RGB888)
    {
...
static bool encode_with_hw_jpeg(const uint8_t *src, size_t src_len, uint16_t width, uint16_t height,
                                pixel_format_t format, uint8_t quality, uint8_t **jpg_out, size_t *jpg_out_len,
...
bool image_to_jpeg(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, pixel_format_t format,
                   uint8_t quality, uint8_t **out, size_t *out_len)
{
#ifdef CONFIG_XIAOZHI_CAMERA_ALLOW_JPEG_INPUT
    if (format == PIX_FMT_JPEG)
    {
...
bool image_to_jpeg_cb(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, pixel_format_t format,
